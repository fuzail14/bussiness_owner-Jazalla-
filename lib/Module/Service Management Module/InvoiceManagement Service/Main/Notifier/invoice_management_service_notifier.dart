...
import '../State/invoice_management_service_state.dart';

class InvoiceManagementServiceNotifier
    extends StateNotifier<InvoiceManagementServiceState> {
  final Person? person;
  final invoiceManagmentRepository = InvoiceManagmentRepository();

  InvoiceManagementServiceNotifier(this.person)
      : super(InvoiceManagementServiceState()) {
    invoiceForBuyerView(
...
final invoiceManagementServiceProvider = StateNotifierProvider.autoDispose<
    InvoiceManagementServiceNotifier, InvoiceManagementServiceState>((ref) {
  final person = ref.read(personProvider);
  if (person == null) {
    throw Exception('Person data is not available');
  }
  return InvoiceManagementServiceNotifier(person);
});
