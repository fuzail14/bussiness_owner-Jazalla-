...
import '../State/client_recieved_inviation_sales_state.dart';

class ClientRecievedInvitationSalesNotifier
    extends StateNotifier<ClientRecievedInviationSalesState> {
  final request4InformationRepository = Request4InformationRepository();
  Person? person;
  ClientRecievedInvitationSalesNotifier({person})
      : super(ClientRecievedInviationSalesState()) {
...
final clientRecievedInvitationProvider = StateNotifierProvider.autoDispose<
    ClientRecievedInvitationSalesNotifier,
    ClientRecievedInviationSalesState>((ref) {
  final person = ref.read(personProvider);
  if (person == null) {
    throw Exception('Person data is not available');
  }
  return ClientRecievedInvitationSalesNotifier(person: person);
